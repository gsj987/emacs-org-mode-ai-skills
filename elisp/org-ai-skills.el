             ...
             (condition-case err
               ...
               (org-ai-skills-planner-error
                (let ((msg (error-message-string err)))
                  ...
                  (cond
                   ((or (string-match-p "End of file while parsing JSON" msg)
                        (string-match-p "does not contain a JSON object" msg))
                    nil)
                   ((and (string-match-p "Planner response must include non-empty plan" msg)
                         (not (plist-get run-state :steps)))
                    (let ((fallback
                           (org-ai-skills--planner-fallback-response-from-metadata
                            metadata)))
                      (if fallback
                          (progn
                            (setq done t)
                            (org-ai-skills--append-debug-entry
                             (list :event-type 'planner-fallback
                                   :request-role 'planner
                                   :task task
                                   :prompt "Applied metadata fallback due to empty planner response"))
